...
export const STATUS_COLS = `
  ...
  COUNT(*) FILTER (WHERE l.walkin_call_status='already_paid')            AS ap,
  COUNT(*) FILTER (WHERE l.walkin_call_status='future_follow_up')        AS ffu,
  COUNT(*) FILTER (WHERE l.walkin_call_status='not_registered')          AS nr,
  COUNT(*) FILTER (WHERE l.walkin_call_status='not_visited')             AS nv,
  COUNT(*) FILTER (WHERE l.walkin_call_status='disqualified')            AS dq,
  COUNT(*) FILTER (WHERE l.walkin_call_status='invalid')                 AS inv,
  COUNT(*) FILTER (WHERE l.walkin_call_status IS NOT NULL AND l.walkin_call_status NOT IN (
    ...
    'future_follow_up','not_registered','not_visited','disqualified','invalid',
  ...
  return {
    text: `
      ...
      SELECT
        ...
        COALESCE(ls.ap, 0)                   AS ap,
        COALESCE(ls.ffu, 0)                  AS ffu,
        COALESCE(ls.nr, 0)                   AS nr,
        COALESCE(ls.nv, 0)                   AS nv,
        COALESCE(ls.dq, 0)                   AS dq,
        COALESCE(ls.inv, 0)                  AS inv,
